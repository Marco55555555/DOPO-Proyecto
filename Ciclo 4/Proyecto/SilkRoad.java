...
public class SilkRoad {
    ...
    /**
     * Coloca una tienda normal en la celda indicada con el precio especificado.
     * Siempre crea NormalStore.
    ...
    public void placeStore(int celdaIndex, int price) {
        if (celdaIndex <= 0 || celdaIndex > celdas.size()) return;
        Store tienda = new NormalStore(price);
        celdas.get(celdaIndex - 1).setStore(tienda);
        updateProgressBar();
    }

    /**
     * Crea y coloca una tienda del tipo indicado en la celda especificada,
     * asignándole el precio dado y actualizando la barra de progreso.
     *
     * @param type       tipo de tienda ("fighter" (purple), "charity" (turquoise), "auto" (pink) o por defecto "normal" (blue))
     * @param price      precio base de la tienda
     */
    public void placeStore(String type, int celdaIndex, int price) {
        if (celdaIndex <= 0 || celdaIndex > celdas.size()) return;
        Store tienda;
        switch (type.toLowerCase()) {
            case "fighter" -> tienda = new FighterStore(price);
            case "charity" -> tienda = new CharityStore(price);
            case "auto" -> {
                tienda = new AutonomousStore(price);
                int randomCell = 1 + random.nextInt(celdas.size());
                celdas.get(randomCell - 1).setStore(tienda);
                updateProgressBar();
                return;
            }
            default -> tienda = new NormalStore(price);
        }
        celdas.get(celdaIndex - 1).setStore(tienda);
        updateProgressBar();
    }
    ...
    /**
     * Método original: coloca un robot normal en la celda indicada.
     * Mantiene la compatibilidad con la versión anterior del juego.
     */
    public void placeRobot(int celdaIndex) {
        ...
        Robot robot = new NormalRobot(); 
        robot.setInitialCellIndex(celdaIndex);
        celda.addRobot(robot);
    }
 
    /**
     * Nuevo método: coloca un robot en la celda indicada, especificando el tipo.
     * @param tipoRobot "normal", "neverback" o "tender"
     * @param celdaIndex número de celda (1-indexado)
     */
    public void placeRobot(String tipoRobot,int celdaIndex) {
        if (celdaIndex <= 0 || celdaIndex > celdas.size()) {
            if (isVisible) {
                JOptionPane.showMessageDialog(null,"Índice de celda inválido: " + celdaIndex);
            }
        ...
        Robot robot;
        switch (tipoRobot.toLowerCase()) {
            case "neverback":
                robot = new NeverBackRobot();
                break;
            case "tender":
                robot = new TenderRobot();
                break;
            default:
    ...
    /**
     * Mueve un robot de la celda indicada la cantidad de pasos.
     * Si cae en una celda con tienda, recoge ganancias según su tipo.
    ...
    public void moveRobot(int celdaIndex, int pasos) { 
        ...
        if (destino.getStore() != null && destino.getStore().canSellTo(robot)) {
            ...
            tienda.sellTo(robot);  
            if (isVisible) {
                JOptionPane.showMessageDialog(null, "El robot ha interactuado con la tienda en la celda " + destino.getRoadPosition());
            }
